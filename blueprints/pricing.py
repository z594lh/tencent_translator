...
def _build_fixed_cost_subtree(cursor, costs, exchange_rate, seller_sku):
    """
    构建固定成本子树，返回 (fixed_cost_usd, fixed_cost_node, headway_detail, waybill_info)
    """
    ...
    return fixed_cost_usd, fixed_cost_node, headway_detail, waybill_info


def _build_rate_nodes(costs, ad_rate, refund_rate):
    """构建佣金、广告、退货三个费率叶子节点"""
    ...
    return commission_node, ad_rate_node, refund_rate_node


def _build_base_result(costs, exchange_rate, fixed_cost_usd, headway_detail, waybill_info, price_result, seller_sku):
    """构建两个接口共用的基础响应字段"""
    return {
        ...
        "fixed_cost_usd": float(fixed_cost_usd.quantize(Decimal("0.01"))),
        "commission": {
        ...
        "cost_breakdown": {
            ...
            "variable_cost_usd": _f(price_result.get("variable_cost")),
...
def _calculate_with_tree(cursor, seller_sku, target_profit_rate, ad_rate, refund_rate,
    ...
    fixed_cost_usd, fixed_cost_node, headway_detail, waybill_info = _build_fixed_cost_subtree(
        cursor, costs, exchange_rate, seller_sku
    )
    ...
    # 构建费率节点
    commission_node, ad_rate_node, refund_rate_node = _build_rate_nodes(costs, ad_rate, refund_rate)
    ...
    result = _build_base_result(costs, exchange_rate, fixed_cost_usd, headway_detail, waybill_info, price_result, seller_sku)
    result["inputs"] = {
    ...
    result["suggested_price"] = _f(price_result.get("suggested_price"))
    result["actual_profit_rate"] = _f(price_result.get("actual_profit_rate"))
    result["calc_tree"] = calc_tree

    return result, None, calc_tree


def _calculate_profit_rate_with_tree(cursor, seller_sku, selling_price, ad_rate, refund_rate,
                                     shop_id=None):
    """
    执行单个 SKU 的利润率反推计算，并返回 (result_dict, error_msg, calc_tree)。
    传 shop_id 则实时调 Amazon API 获取准确费率，不传则走缓存表。
    """
    exchange_rate = get_exchange_rate(cursor, 'CNY', 'USD')
    if shop_id is not None:
    ...
    price_result = calculate_profit_rate(
        fixed_cost_usd=fixed_cost_usd,
    ...
    if price_result.get("calc_note"):
        tree = _build_calc_node(
            "profit_rate", VARIABLE_LABELS.get("profit_rate"), None,
            "profit_amount / selling_price",
            is_leaf=0,
            children=[_build_calc_node(
                "calc_error", VARIABLE_LABELS.get("calc_error"), price_result["calc_note"],
                f"selling_price = {float(selling_price):.2f} ≤ 0",
                is_leaf=1)]
        )
        return None, price_result["calc_note"], tree

    profit_rate = price_result['profit_rate']

    selling_price_node = _build_calc_node(
        "selling_price", VARIABLE_LABELS.get("selling_price"),
    ...
    calc_tree = _build_calc_node(
        "profit_rate", VARIABLE_LABELS.get("profit_rate"),
        f"{float(profit_rate):.4f}",
        f"1 - fixed_cost_usd({float(fixed_cost_usd):.2f})/selling_price({float(selling_price):.2f}) - commission_rate({float(costs.commission_rate):.4f}) - ad_rate({float(ad_rate):.4f}) - refund_rate({float(refund_rate):.4f})",
        is_leaf=0,
        children=[selling_price_node, fixed_cost_node, commission_node, ad_rate_node, refund_rate_node],
    )
    ...
    result["profit_rate"] = _f(price_result.get("profit_rate"))
    result["calc_tree"] = calc_tree
...
@pricing_bp.route('/pricing/profit-rate', methods=['POST'])
@login_required
@permission_required('pricing:calculate')
def calculate_profit_rate_api():
    """
    售价反推利润率接口（含计算过程树）
    请求体:
      seller_sku      必填  SKU
    ...
    try:
        ...
        if selling_price_raw is None:
            return jsonify({"status": "error", "message": "selling_price 不能为空"}), 400
        try:
            selling_price = Decimal(str(selling_price_raw))
        except (ValueError, TypeError):
            return jsonify({"status": "error", "message": "selling_price 必须是数字"}), 400
        ...
        try:
            with conn.cursor() as cursor:
                ...
                result, err, calc_tree = _calculate_profit_rate_with_tree(
                    cursor, seller_sku, selling_price, ad_rate, refund_rate,
                    shop_id=shop_id
                )
